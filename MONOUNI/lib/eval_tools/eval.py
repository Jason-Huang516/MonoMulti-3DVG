...
def calculate_iou_partly(gt_annos,
    ...
    for num_part in split_parts:
        gt_annos_part = gt_annos[example_idx:example_idx + num_part+1]
        dt_annos_part = dt_annos[example_idx:example_idx + num_part+1]
        if metric == 0:
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for j, num_part in enumerate(split_parts):
                    gt_datas_part = np.concatenate(
                        gt_datas_list[idx:idx + num_part+1], 0)
                    dt_datas_part = np.concatenate(
                        dt_datas_list[idx:idx + num_part+1], 0)
                    dc_datas_part = np.concatenate(
                        dontcares[idx:idx + num_part+1], 0)
                    ignored_dets_part = np.concatenate(
                        ignored_dets[idx:idx + num_part+1], 0)
                    ignored_gts_part = np.concatenate(
                        ignored_gts[idx:idx + num_part+1], 0)
                    fused_compute_statistics(
                        parted_overlaps[j],
                        pr,
                        total_gt_num[idx:idx + num_part+1],
                        total_dt_num[idx:idx + num_part+1],
                        total_dc_num[idx:idx + num_part+1],
                        gt_datas_part,
                        dt_datas_part,
                        dc_datas_part,
                        ignored_gts_part,
